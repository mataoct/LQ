...
+(void)showErrorWithStatus_custom:(NSString *)string duration:(NSTimeInterval)duration
{
    [[SVProgressHUD sharedView] showWithStatus:nil maskType:SVProgressHUDMaskTypeNone networkIndicator:NO];
    [SVProgressHUD dismissWithError:string afterDelay:duration];
}

+ (void)showSuccessWithStatus_custom:(NSString *)string duration:(NSTimeInterval)duration
{
    [[SVProgressHUD sharedView] showWithStatus:nil maskType:SVProgressHUDMaskTypeNone networkIndicator:NO];
    [SVProgressHUD dismissWithSuccess:string afterDelay:duration];
}
...
+(void)dismiss:(int)intdelay
{
    [[SVProgressHUD sharedView] dismiss:intdelay];
}
+ (void)dismiss {
...
- (void)dismiss:(int)intdelay
{
    self.fadeOutTimer = [NSTimer scheduledTimerWithTimeInterval:intdelay target:self selector:@selector(dismiss) userInfo:nil repeats:NO];
}
- (void)dismiss {
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.15
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
		...
		stringLabel.textAlignment = UIBaselineAdjustmentAlignCenters;
